...
def stopAllPlayers(players):
    ...
    for station in players:
        player = players[station]
        if checkIsPlaying(player) is True:
            player.stop()
            stopped.append(station)
    return stopped


def getVLCMeta(player):
    return True
...
# get the track name from WYMS's XML file. This is specific to WYMS for now
# because there is no standard for how these XPSF files are meant to work. It
# could be different for other stations or even Radio Milwaukee streams.
#
# There are admittedly a lot of assumptions going on here, the main one being
# that the track information is the third item's (root[2]) only child's
# (root[2][0]) second child (root[2][0][1]).
