...
def getStationData(stations, request, players):
    ...
    for station in stations:
        is_playing = streamRadio.getStatus(players[station])
        streams[station] = {
...
def radioStreamQueryArgs(request, status, player, UNSUPPORTED, PLATFORM):
    ...
    if 'play' in request.args.keys():
        if not player.is_playing():
            player.play()
            # Wait briefly for VLC to open the stream so we can report
            # a real outcome ("playing" or "error") instead of guessing.
            outcome = streamRadio.waitForOutcome(player)
            status['status'] = outcome
            if outcome == "error":
                status['message'] = (
                    "VLC could not open the stream URL. The URL may be "
                    "dead or unreachable — check the journalctl logs for "
                    "'unable to open the MRL'."
                )
